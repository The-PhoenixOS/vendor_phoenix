...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('ProjectFlare/%s already fetched to %s' % (repo_name, repo_target))
            continue

        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "ProjectFlare/%s" % repo_name,
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/Project-Flare/" + repo_name + "/branches")
    add_auth(githubreq)
